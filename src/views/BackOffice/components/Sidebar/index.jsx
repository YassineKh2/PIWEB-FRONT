...
import { RiTeamFill as Team} from "react-icons/ri";
import { User } from 'lucide-react';
import { RiTeamFill as StadiumIcon } from "react-icons/ri";
...
const Sidebar = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  return (
    <aside
      ref={sidebar}
      className={`absolute left-0 top-0 z-9999 flex h-screen w-72.5 flex-col overflow-y-hidden bg-backofficeblack duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
 ...
 {/* <!-- Menu Item stadiums --> */}
 <SidebarLinkGroup
  activeCondition={pathname.includes('stadiums')}
>
  {(handleClick, open) => (
    <>
      <NavLink
      ...
      >
        <StadiumIcon size={20} /> {/* Use the stadium icon here */}
        Stadiums
        <svg
          className={`absolute right-4 top-1/2 -translate-y-1/2 fill-current ${
            open && 'rotate-180'
 ...
 {/* <!-- Menu Item stadiums --> */}
              ...
              <li>
                <NavLink
                  to="./profile"
                  className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    pathname.includes('Ticket') && 'bg-graydark dark:bg-meta-4'
                  }`}
                >
                  <svg
                    className="fill-current"
